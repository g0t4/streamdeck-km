...
export class KeyboardMaestroHelper {
   ...
   */
  static async getMacroList(): Promise<MacroGroup[]> {
    try {
      ...
      const groups: MacroGroup[] = JSON.parse(stdout.trim());
      return groups;
    } catch (error: any) {
   ...
   */
  private static async getMacroListFallback(): Promise<MacroGroup[]> {
    try {
   ...
   */
  private static parseSimpleOutput(output: string): MacroGroup[] {
    const groups: MacroGroup[] = [];
    let currentGroup: MacroGroup | null = null;
